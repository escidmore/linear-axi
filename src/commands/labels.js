...
import { collectKnownArgs, dispatchCommandGroup, rejectIdOnCreate, rejectUnknownInput, requireValue } from "../lib/cli-helpers.js";
...
async function createLabelCommand(args, runtime) {
  ...
  rejectUnknownInput(parsed, LABEL_MUTATION_FIELDS.filter((name) => name !== "id"), LABEL_CREATE_HELP);
...
async function updateLabelCommand(args, runtime) {
  ...
  rejectUnknownInput(parsed, LABEL_MUTATION_FIELDS, LABEL_UPDATE_HELP);
  const toolArgs = labelToolArgs(parsed);
...
async function deleteLabelCommand(args, runtime) {
  ...
  rejectUnknownInput(parsed, ["id"], LABEL_DELETE_HELP);
